...
class Speech2Text:
    ...
    def __init__(
        ...
        # Add:: Acoustic Adaptive Scorer
        acoustic_adaptive_weight: float = 1.0,
    ):
        if enh_s2t_task:
            try:
        ...
        # ---------------------------------------------------------
        # Acoustic Adaptive Scorer
        #
        # The scorer is already included in the integrated
        # ESPnet ASR checkpoint.
        # ---------------------------------------------------------

        acoustic_adaptive_scorer = None

        if (
            getattr(
                asr_model,
                "use_acoustic_adaptive",
                False,
            )
            and getattr(
                asr_model,
            ...
            acoustic_adaptive_scorer = (
                asr_model.acoustic_adaptive
            )

            scorers["acoustic_adaptive"] = (
                acoustic_adaptive_scorer
            )

            logger.info(
                "Acoustic Adaptive Scorer enabled "
                "from integrated ASR model: "
                f"weight={acoustic_adaptive_weight}"
            )
        ...
        if asr_model.use_transducer_decoder:
            ...
            if acoustic_adaptive_scorer is not None:
                weights["acoustic_adaptive"] = (
                    acoustic_adaptive_weight
                )
    ...
    # def __call__(self, speech: Union[torch.Tensor, np.ndarray]) -> Union[
        ...
        if self.acoustic_adaptive_scorer is not None:
            if "acoustic_feat" not in batch:
                raise RuntimeError(
                    "Acoustic Adaptive is enabled, "
                    "but acoustic_feat was not provided."
                )
            (enc, enc_olens, condition_vector) = self.asr_model.encode(**batch, return_condition_vector=True)

            # Use the SAME vector generated by
            # Acoustic Conditioning.
            self.acoustic_adaptive_scorer.set_condition_vector(condition_vector)
        else:
            enc, enc_olens = self.asr_model.encode(**batch)
        ...
        if self.acoustic_adaptive_scorer is not None:
            self.acoustic_adaptive_scorer.clear_condition_vector()
...
def inference(
    ...
    # Add:: Independent Acoustic Adaptive Scorer
    acoustic_adaptive_weight: float,
):
    if batch_size > 1:
    ...
    speech2text_kwargs = dict(
        ...
        max_seq_len=max_seq_len,
        max_mask_parallel=max_mask_parallel,
        # Independent Acoustic Adaptive Score
        acoustic_adaptive_weight=(
            acoustic_adaptive_weight
        ),
    )
...
def get_parser():
    ...
    group.add_argument(
        ...
        default=1.0,
        help=(
            "Weight of integrated Acoustic Adaptive "
            "score during beam search"
        ),
    )
    return parser
